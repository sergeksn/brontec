...
import { wait, request_to_server } from "./../libs/func-kit.js";
import { set_cookie, get_cookie, delete_cookie } from "./../libs/cookie.js";
...
export default new class {
    ...
    async open_results_block() {
        ...
        let lower_header = anime({ //опускаем хедер до низа окна бразуера
            ...
            lower_search_results_block = anime({ //опускаем блок с результатами поиска да низа окна браузера
        ...
        anime({ //показываем лоадер после откытия блока с результатами поиска
    ...
    async close_results_block(full_close = false) {
        ...
        //если есть какие-то отображённые результаты поиска
            ...
            await anime({ //дожидаемся пока станет прозрачным блок с результатами поиска
        ...
        await anime({ //уменьшаем высоту блок с результатами поиска для его скрытия
            targets: this.search_results[0],
            height: 0,
            duration: GDS.anim_time,
            easing: GDS.anim_tf
    ...
    async render_results(search_text) {
        ...
        if (this.results_wrapper[0].innerHTML !== "") {
            ...
            await anime({ //дожидаемся пока результаты поиска станут прозрачными
                targets: this.results_wrapper[0],
                opacity: 0,
                duration: GDS.anim_time,
                easing: GDS.anim_tf
        ...
        let show_results = async (result_html) => {
            ...
            await anime({ //дожидаемся скрытия лоадера
                targets: this.search_loader[0],
                opacity: 0,
                duration: GDS.anim_time,
                easing: GDS.anim_tf
            ...
            await anime({ //плавно показываем блок с результатами
                targets: this.results_wrapper[0],
                opacity: 1,
                duration: GDS.anim_time,
                easing: GDS.anim_tf
