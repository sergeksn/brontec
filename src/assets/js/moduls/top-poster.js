...
GDS.header.top_baner = {
    ...
    hide_baner: async function(cookie = true) {
        let baner_id = this.top_baner.attr("id"),
            ...
            header_background_height = header_h - baner_height; //высота для подложки хедера
        ...
        await Promise.all([
            ...
            $("#header-background").animate({ "height": header_background_height + "px" }, GDS.anim_time, GDS.anim_tf), //плавно поднимаем подложку хедера и после того как анимация закончится вызываем колбек
