...
let wrap = qs('.template-s-plusikami'),
    ...
    CONTROLLER = {
        ...
        touched_init: function (el) {
            qs('.template-s-plusikami-plusik-icon', el)._on('touchend', () => {
        ...
        set_pop_up_position: function () {
            all_plusiki.forEach(el => {
                let ww = GDS.win.width, //ширина окна браузера
                    pop_up = qs('.template-s-plusikami-plusik-pop-up', el); //окно с ссылкой

                pop_up.style.display = 'block';//отображаем попапы чтоб можно было адекватно измерять его размеры и позицию

                let icon_plusik = qs('.template-s-plusikami-plusik-icon', el), //иконка плюсика в синей обводке
        ...
        init: function () {
            ...
            all_plusiki = qsa('.template-s-plusikami-plusik', wrap); //все интерактивные элементы плюсиков

            w._on('load', this.set_pop_up_position.bind(this)); //позиционируем всплывающие подсказки
