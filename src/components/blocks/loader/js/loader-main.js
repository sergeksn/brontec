...
class Loader {
    ...
    //показываем лоадер
    async show() {
        if (!this.permission_to_show) throw 'not permission to show'; //запрещено показывать
        if (!this.loader) throw 'no loader'; //нет лоадера
        if (this.status === 'show') throw 'already is show'; //лоадер уже виден
        if (this.status === 'pending to show') throw 'already is pending to show'; //уже в процессе появления
        ...
        try {
            ...
            throw e; //выбрасываем исключение чтоб во внешнем коде выполнение перешло к первому блоку catch
        }
    }
    //показываем лоадер
    ...
    //скрываем лоадер
    async hide() {
        if (!this.permission_to_hide) throw 'not permission to hide'; //запрещено скрывать
        if (!this.loader) throw 'no loader'; //нет лоадера
        if (this.status === 'hide') throw 'already is hide'; //лоадер уже скрыт
        if (this.status === 'pending to hide') throw 'already is pending to hide'; //уже в процессе скрытия
