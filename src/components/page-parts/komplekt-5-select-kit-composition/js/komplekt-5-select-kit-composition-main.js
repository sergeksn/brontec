...
import { Header_Cart, Header } from '@header-main-js';
...
let configurator = qs('.komplekt-configurator') ?? qs('.detal-1-add-to-kit'), //блок с конфигуратором на странице комплекта ли детали
    detal_single_input = qs('.detal-1-add-to-kit input'), //инпут страницы детали
    to_configurator_url_string = '#komplekt-configurator',//строка в адресе которая сигнализирует что мы перешли целеноправленно на конфигуратор
    ...
    top_block_button_add_full_kit = qs('.komplekt-1__info-buttons-to-cart'), //кнопка для добавления комплекта полностью сверху
    button_add_configuration_kit = qs('.komplekt-configurator__wrap-composition-button'), //кнопка для добавленяи в корзину сконфигурированного из частей комплекта
    bottom_block_button_add_full_kit = qs('.komplekt-10__wrap-composition-button'), //кнопка для добавления комплекта полностью снизу
    CONFIGURATOR_CONTROLLER = {
        ...
        add_or_go_to_cart: async function (e) {
            ...
            if (path.find(el => el?.classList?.contains('komplekt-configurator--nothing-selected'))) return; //если кнопка дял конфигуратора и ничего не выбрано в конфигураторе то ничего не делаем при нажатии на кнопку, данняа пометка может быть только у конфигуратора но неу  полного комплекта
            ...
            if (button.dataset.inCart == 'no') {
                let is_full_kit_button = path.find(el => el?.classList?.contains('komplekt-10')) || path.find(el => el?.classList?.contains('komplekt-1')), //проверяем что это за кнопка нажата
        ...
        init: async function () {
            ...
            if (!qs('.komplekt-configurator')) return; //если нет конфигуратора полного комплекта прерываем

             //если мы перешли на страницу целенаправлино на блок конфигуратора
             if (location.hash == to_configurator_url_string) {
                ...
                //history.pushState(null, '', location.href.replace(to_configurator_url_string, '')); //делаем в истории браузера ссылку на эту страницу но уже без примиски #send-request-for-new-car чтоб при обновлении не перекидывало на конфигуратор, хз обязательно это или нет не тестил во всех браузерах, пусть лучше будет =)
            }
            //если мы перешли на страницу целенаправлино на блок конфигуратора
            ...
            [top_block_button_add_full_kit, button_add_configuration_kit, bottom_block_button_add_full_kit].forEach(button => button._on('click', this.add_or_go_to_cart.bind(this))); //при нажатии на кнопки добавлени/перехода в корзину у полного комплекта и конфигуратора комплекта
