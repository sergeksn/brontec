...
export default new (class {
    constructor() {
        ...
        this.cart_overlay = document.getElementById('cart-overlay');
        this.header_visible = d.querySelector('.header-visible'); //постоянно видимая часть меню
        this.header_cart_button = d.querySelector('.header-visible__cart-button>button');
        this.body = d.getElementsByTagName('body')[0];
        this.header = d.getElementsByTagName('header')[0];

        this.Overlay = new Overlay({ el: d.getElementById('cart-overlay') }); //создаём экземпляр подложки корзины

        this.lock = false;
        this.status = 'hide';

        [this.header_cart_button, this.cart_overlay].forEach(item => item._on('click', e => this.toggle_cart())); //показываем/скрываем корзину при клике
    ...
    async open_cart() {
        ...
        this.header.style.overflowY = 'hidden'; //блокируем прокуртку хедера
        ...
        //взаимодействуем с подложкой корзины только на экрнах больше или равных 640px
        let action_overlay = GDS.win.width_rem < 40 ? null : this.Overlay.show();

        await Promise.all([this.show(), action_overlay, Scroll_To_Top_Button.hide()]); //дожидаемся показа корзины и подложки, а так же скрытия кнопки скрола вверх
    }
    //выпоялняем все действия для открытия корзины
    ...
    size_recalculate() {
        ...
        if (this.status === 'show') GDS.win.width_rem < 40 ? this.Overlay.hide() : this.Overlay.show(); //если корзина видна то для экранов меньше 640px скрываем подложку при ресайзах, а для экранов боле показываем
    }
    //пересчитываем верхний отступ корзины пре ресайзе
})();
