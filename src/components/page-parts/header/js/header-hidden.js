...
let body = qs('body'),
    ...
    CONTROLLER = {
        ...
        //ПРИМЕЧАНИЕ: так же если в локальном хранилище есть поисковой запрос откроет блок с результатами поиска
        toggle_header_hidden_block: async function () {
            ...
            await Header.show(); //пытаем показать хедер, т.к. клик мог произойти в момент когда хедер в движении после скрола, в этом случае мы дождёмся пока хедер полностью не покажется, сели же он уже был полностью виден этот пункт завершится мгновенно
            ...
            if (this.status === 'close') {
                ...
                if (search_text) {
                    search_input.value = search_text; //записываем текст поиска в инпут
                    search_input.classList.add('started-inputed'); //помечаем что в инпуте есть текст
                }
                //если в локальном хранилище есть сохранённый поисковой запрос

                await this.open(); //открываем блок

                if (!GDS.device.is_touch) search_input.focus({ preventScroll: true }); //ставим курсор на наше поле ввода НЕ на сенсорных экранах
        ...
        init: function () {
            [
                ...
                el._on('click', () => this.toggle_header_hidden_block()); //открываем/закрываем скрытый блок при клике на бургер кнопку, кнопку поиска в видимой части хедеера или на подложку хедера и кнопку закрытия поиска
            });
