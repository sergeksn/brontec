...
let header = qs('header'),
    ...
    CONTROLLER = {
        ...
        where_add_instructon: 'komplekts', //указываем где добавляем инструкции, возможные варианты:
        ...
        render_results: async function (search_text) {
            ...
            await this.load_results(search_text)
                ...
                .catch(async e => {
                    console.error(e);
                    await show_results('<div class="header-search__fail">Приносим извинения, произошла ошибка, попробуйте пожалуйста позже =(</div>'); //вставляет переданные код html в блок с результатами поиска и плавно показывает его
                }); //этот блок catch может сработать только из-за прерывания поиска AbortError или при ошибке в коде или исключении внутри then
            //ищем search_text - текст введённый в поле поиска в базе, после того как получили ответ выводим его на экран
        },
        //раскрываем окно с результатами поиска и отображает результаты поиска в блоке для результатов
                ...
                await request_to_server(request_data)
                    ...
                    .catch(e => {
                        if (typeof e.ksn_message === 'undefined') {
                            console.error(e);
                            return reject(e);
                        }; //если ошибка не наша выводим её в консоль
