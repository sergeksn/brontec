...
const Img_Loader = new (class {
    ...
    constructor() {
        ...
        $(window).on({
            ...
            callback: async () => {
                ...
                $("[data-img-type]:not(.portrait-uploaded.landscape-uploaded):not(.uploaded)").each(el => temporary.observe(el)); //добавляем все элементы на отслеживания видимости во временный IntersectionObserver, ВАЖНО добавляем именно те элементы которые ещё не загружены или загружены только в одной ориентации
            },
        });
        //проверяем видимость картинок при смене ориентации
    ...
    get_img_size_url(img) {
        ...
        if (current_size) {
            if (current_size === "original" || Number(current_size) >= miniatura_width) return "no need to update"; //если размер текущей картинку уже её оригинал, или текущий размер миниатюры такой же как и тот что мы хотим сейчас установить то завершаем функцию
        }
        //если размер для противоположной ориентации уже был задан
        ...
        return url_bez_extension + "-" + miniatura_width + "x" + data_miniatura_height + extension; //возвращаем расчитаный url для миниатюры wp-content/uploads/2021/03/1-2-2000x702.jpg к примеру
    }
    //получаем адрес миниатюры картинки с учётом ширины картинки на сайте
    ...
    //функция для загрузки картинок из блоков image-wrap где всего одна картинка, вставленная через тег img или через css background-image
    //img_wrap - image-wrap текущей картинки
    ...
    common_img_loader(img_wrap, bg = false) {
        let search_classes = [`${GDS.device.orientation}-started_loaded`, `${GDS.device.orientation}-uploaded`, "started-loaded", "uploaded"]; //список классов которые сигнализируют о том что дальнейшие операции нужно прервать так какртинка уже загружена или впроцессе загрузки, в случае НЕ SVG загружена для текущей ориентации

        if ([...img_wrap.classList].some(className => search_classes.includes(className))) return; //проверяем если среди классов img_wrap классы из search_classes, если есть хоть один из них то прерываем функцию

        img_wrap = $(img_wrap); //получаем img_wrap в оболочке ksn

        let img_el = img_wrap.find(".picture__body"), //элемент содержащий картинку
            img_extension = img_el.attr("data-src").match(/\.{1}([a-zA-Z]+)$/)[1], //узнаём формат изображения
            current_orientation = GDS.device.orientation, //ориетация устройства в момент начала загрузки
            is_svg = img_extension === "svg", //указывает svgтинка или нет
            loader = img_wrap.find(".loader"), //лоадер для данной картинки
        ...
        if (url === "no need to update") {
            img_wrap.addClass(`${current_orientation}-uploaded`); //помечаем что данная картинка загружена и в этой ориентации
        ...
        is_svg ? img_wrap.addClass("started-loaded") : img_wrap.addClass(`${current_orientation}-started-loaded`); //помечаем что загрузка данной картинки для данной ориентации или просто картинка в случае svg началась чтоб не начинать её грузить снова и снова пока она всё ещё не загружена

        //ждйм завершения загрузки картинки по url
        this.download_tracker(url)
            .then(() => {
                if (is_svg) {
                    img_wrap.addClass("uploaded").removeClass("started-loaded"); //в зависимости от формата картинки добавляем нужные классы загрузки и удаляем класс процесса загрзки
                    ...
                    img_wrap.addClass(`${current_orientation}-uploaded`).removeClass(`${current_orientation}-started-loaded`); //в зависимости от формата картинки добавляем нужные классы загрузки и удаляем класс процесса загрзки

                    if (img_wrap.hasClass("portrait-uploaded landscape-uploaded")) this.img_visible_observer.unobserve(img_wrap[0]); //если картинка была загружена во всех ориентация то можем удалить её из наблюдателя за видимостью элементов
                }
                ...
                if (loader.length > 0) {
                    ...
                    anime({
                        ...
                        easing: "linear",
                        complete: () => {
                            bg ? img_el.css("background-image", `url(${url})`) : img_el.attr("src", url); //в зависмости от того какой тип картинки фоновая или встроеная мы либо задём фон элементу или задаём атрибут src для img
                            loader.css("display", "none"); //убираем данный лоадер из вывода
                        },
                    });
                    //анимируем сокрытие лоадера, а после его сокрытия показываем картинку
                }
    ...
    add_size_img_wrap_product_previv_block() {
        ...
        $(".product_previv_block").each(block => {
            let product_previv_block = $(block), //текущий блок
                ...
                previv_images_wraper = product_previv_block.find(".previv_images_wraper"), //оболочка блоков с картинками продукта и стрелочки
                product_prevu_img_block = product_previv_block.find(".product_prevu_img_block"); //оболочка картинки машины и её стставлющих svg

            let pecent_h_from_w = Number(product_prevu_img_block.css("padding-top").replace("px", "")) / (product_prevu_img_block.width() / 100), // получаем сколько процентов составляет высота блока product_prevu_img_block от ширины его родителя
